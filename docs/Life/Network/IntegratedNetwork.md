...
- [**校园手机卡**](/Life/Network/CampusMobilePhoneCard)：即在校园内三大运营商处办理的SIM卡。

- [**校园VPN**](/Life/Network/VPN)：用于在公用网络上建立专用网络，进行加密通讯，保证数据安全，访问校内资源。

- [**校园5G专网**](5GCampusPrivateNetwork)：学校与运营商合作，为师生提供专线网络访问校内资源。
...
1. Q：连校园网再连学校VPN，也能上外网？

    A：~~据说登录时选择**校园网**，再连接学校VPN，就可以实现访问互联网的目的。不过速率堪忧。~~

    **此方法已在2024年秋季被禁止，不再适用。[详情见同目录下 VPN 页面](https://live.cumt.asia/Life/Network/VPN.html)。**
...
- 无线：一体化网络的无线信号已基本做到覆盖全校的建筑物，包括宿舍、食堂、图书馆、公教楼、学院楼、大创中心、行政楼等。实测下载速率最高能达到20MB/s+。
  ...
  部分位置，如由于宿舍楼中AP设备安装在宿舍里，因此在宿舍楼道，信号可能会比较一般。

  无法安装AP的地方，如马路、操场等较为宽阔场地可能会有 Wifi 信号盲区。
...
无论哪种形式，同一统一身份认证账号使用一体化网络，总计可以同时连接3个设备。若对同时连接设备的数量有疑问，请联系营业厅客服处理。

## 使用路由器

可以通过有线连接或者无线中继的方法使用路由器(鉴于稳定性和速率原因，推荐有线方式连接)。通过路由器连接一体化网络有两种模式：桥接与路由。

- 路由模式下，连接到路由器的所有设备和路由器共享一个公网IP地址，可以实现多设备同时上网，只需登录一遍，可以共享同一账号。
